...
class Uploader
{
     ...
     * @param bool $watermark 是否添加水印，默认为false
     */
...
//        $this->stateMap['ERROR_TYPE_NOT_ALLOWED'] = iconv('unicode', 'utf-8', $this->stateMap['ERROR_TYPE_NOT_ALLOWED']);
     ...
     * $w_pos   int     水印位置安排（1-10）【1:顶端居左；2:顶端居中；3:顶端居右；4:中部居左；5中部居中；6：中部居右；7：底端居左；8：底端居中；9：底端居右...值空:随机位置】
     * $w_img   string  水印图片路径
     * $w_text  string  显示的文字
     * $w_font  int     字体大小
     * $w_color string  字体颜色
    ...
    public function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = '新合鑫集团', $w_font = 20, $w_color = '#000000')
    {
        $this->w_img = Yii::getAlias('@webroot').'/theme/images/watermark.png'; //水印图片
        //$this->w_img = $w_img;
        $this->w_pos = 9;
        ...
        $this->w_pct = 85; //透明度
        $font = Yii::getAlias('@webroot') . '/theme/images/fz.ttf'; //字体
        ...
        if (!empty($w_img) && file_exists($w_img)) { //水印图片有效
            ...
            $ifwaterimage = 0;
            $temp = imagettfbbox(ceil($w_font * 2.5), 0, $font, $w_text); //imagettfbbox返回一个含有 8 个单元的数组表示了文本外框的四个角
            $width = $temp[2] - $temp[6];
        ...
        if ($ifwaterimage==1) {
            ...
            if (!empty($w_color) && (strlen($w_color) == 7)) {
                ...
                $color = imagecolorallocate($source_img, $r, $g, $b);
            } else {
                return;
            }
            //imagestring($source_img, $w_font, $wx, $wy, $w_text, imagecolorallocate($source_img, $r, $g, $b));
            imagettftext($source_img, $w_font, 0, $wx, $wy, $color, $font, $w_text);
        }
